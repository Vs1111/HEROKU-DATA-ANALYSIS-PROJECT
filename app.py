...
if user_menu == 'BATTING-WISE ANALYSIS':
    st.title("Most Runs:-")
    boling_file= pd.read_excel("real ipl datset\\best   bolling.xlsx" ,1)
    boling_file=boling_file.set_index(['Rank'])
    boling_file.index.name = 'Rank'
    st.dataframe(boling_file)
    st.title("Most Sixes:-")
    boling_file = pd.read_excel("real ipl datset\\best   bolling.xlsx", sheet_name='Most Sixes')
    boling_file = boling_file.set_index(['Rank'])
    st.dataframe(boling_file)
    st.title("Most Fours:-")
    boling_file = pd.read_excel("real ipl datset\\best   bolling.xlsx",sheet_name='Most Fours')
    boling_file = boling_file.set_index(['Rank'])
    st.dataframe(boling_file)
...
if user_menu == 'TEAM-WISE ANALYSIS':
    ...
    var1 = pd.read_excel("real ipl datset\\filter.xlsx", 'Highest Totals')
    var2 = pd.read_excel("real ipl datset\\filter.xlsx", 'Lowest Totals')
    var3 = pd.read_excel("real ipl datset\\filter.xlsx", 'Wicketkeeping-Most career catch')
    var4 = pd.read_excel("real ipl datset\\filter.xlsx", 'Wicketkeeping-Most career stump')
    var5 = pd.read_excel("real ipl datset\\filter.xlsx", 'Fielding-Most career catches')
    var6 = pd.read_excel("real ipl datset\\filter.xlsx", 'AllRounder500 Runsand 20 wicket')
    var7 = pd.read_excel("real ipl datset\\filter.xlsx", 'most-matches-of-player')
    var8 = pd.read_excel("real ipl datset\\filter.xlsx", 'Most matches as captain')
    var9 = pd.read_excel("real ipl datset\\filter.xlsx", 'Most runs as captain')
    var10 = pd.read_excel("real ipl datset\\filter.xlsx", 'Most man of the match awards')
    var11=pd.read_excel("real ipl datset\\filter.xlsx", 'Most matches won as a captain')
...
if user_menu == 'BOLING-WISE ANALYSIS':
    import plotly.figure_factory as ff
    var12 = pd.read_excel("real ipl datset\\best_boweler.xlsx")
