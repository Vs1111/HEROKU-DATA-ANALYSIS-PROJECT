...
if user_menu == 'BATTING-WISE ANALYSIS':
    st.title("Most Runs:-")
    boling_file= pd.read_excel("realipldatset\\best   bolling.xlsx" ,1)
    boling_file=boling_file.set_index(['Rank'])
    boling_file.index.name = 'Rank'
    st.dataframe(boling_file)
    st.title("Most Sixes:-")
    boling_file = pd.read_excel("realipldatset\\best   bolling.xlsx", sheet_name='Most Sixes')
    boling_file = boling_file.set_index(['Rank'])
    st.dataframe(boling_file)
    st.title("Most Fours:-")
    boling_file = pd.read_excel("realipldatset\\best   bolling.xlsx",sheet_name='Most Fours')
    boling_file = boling_file.set_index(['Rank'])
    st.dataframe(boling_file)
    import plotly.figure_factory as ff
    df = pd.read_csv("realipldatset\Players18.csv")
    df = pd.DataFrame(df)
    st.dataframe(df)
    df = df.iloc[:, 1:]
...
if user_menu == 'OVERALL ANALYSIS':
    ...
    df1 = pd.read_csv("realipldatset\matches.csv")
    ...
    data = pd.read_excel("realipldatset\\heatmap.xlsx",sheet_name='Sheet3')
...
if user_menu == 'TEAM-WISE ANALYSIS':
    ...
    var1 = pd.read_excel("realipldatset\\filter.xlsx", 'Highest Totals')
    var2 = pd.read_excel("realipldatset\\filter.xlsx", 'Lowest Totals')
    var3 = pd.read_excel("realipldatset\\filter.xlsx", 'Wicketkeeping-Most career catch')
    var4 = pd.read_excel("realipldatset\\filter.xlsx", 'Wicketkeeping-Most career stump')
    var5 = pd.read_excel("realipldatset\\filter.xlsx", 'Fielding-Most career catches')
    var6 = pd.read_excel("realipldatset\\filter.xlsx", 'AllRounder500 Runsand 20 wicket')
    var7 = pd.read_excel("realipldatset\\filter.xlsx", 'most-matches-of-player')
    var8 = pd.read_excel("realipldatset\\filter.xlsx", 'Most matches as captain')
    var9 = pd.read_excel("realipldatset\\filter.xlsx", 'Most runs as captain')
    var10 = pd.read_excel("realipldatset\\filter.xlsx", 'Most man of the match awards')
    var11=pd.read_excel("realipldatset\\filter.xlsx", 'Most matches won as a captain')
...
if user_menu == 'BOLING-WISE ANALYSIS':
    import plotly.figure_factory as ff
    var12 = pd.read_excel("realipldatset\\best_boweler.xlsx")
